...
const Container = Styled.div`
    ...
    background-color: ${(props) =>
      props.isDragging ? "#EEEEEE" : "rgba(230,230,230, 1)"};
    border-radius: 2px;
    min-height: 50px;
            ...
            <Card className="task-card">
              <Card.Body className="task-card-body">
                ...
                <DeadlineContainer backColor={task.status === "Done"}>
                  {task.deadline.getDate().toString() +
                    " " +
                    task.deadline.toLocaleString("en-US", { month: "short" })}
                </DeadlineContainer>
                ...
                <UsersContainer>
                  {task.users.map((val, idx) => {
                    return (
                      <TaskMemberContainer coloridx={idx % 10}>
