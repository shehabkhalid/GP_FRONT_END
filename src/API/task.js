...
const postTask = async (token, data) => {
    try {
        const result = await instance.post('/tasks',{
            name: data.name,
...
const addMemberToTask = async (token, taskId, data) => {
    try {
        const result = await instance.patch('/tasks/add/' + taskId, {
...
const removeMemberFromTask = async (token, taskId, data) => {
    try {
        const result = await instance.patch('/tasks/remove/' + taskId, {
            memberId: data.memberId
        }, {
            headers: {
                Authorization: 'Bearer ' + token
...
const addComment = async (token, taskId, data) => {
    try {
        const result = await instance.patch('/tasks/comment/add/' + taskId, {
