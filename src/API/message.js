...
const postTextMessage = async (token , data) => {
    try {
        const result = await instance.post('/messages', {
...
const postImageMessage = async (token, data, image) => {
    try {
        ...
        const result2 = await instance.patch('/messages/picture/' + result.data._id, fdata, {
            headers: {
                Authorization: 'Bearer ' + token
            }
        });
...
const updateTextMessage = async (token, messageId, data) => {
    try {
        const result = await instance.patch('/messages/' + messageId, {
            content: data.content
        }, {
            headers: {
                Authorization: 'Bearer ' + token
